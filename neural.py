...
# Altcoins also work
data = web.DataReader(
    f'{crytocurrency}-{Against_currency}', 'yahoo', start, end)
...
test_data = web.DataReader(
    f'{crytocurrency}-{Against_currency}', 'yahoo', test_start, test_end)
...
plt.title(f'{crytocurrency} price prediction')
plt.xlabel("time")
plt.ylabel('prices')
...
# predict next_day
real_data = [model_inputs[len(model_inputs)+2 -
